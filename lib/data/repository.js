...
class Repository {
    ...
    async createPullRequest(pullRequestTitle, sourceBranch, targetBranch) {
        ...
        if (active.length === 1) //return active.at(0)
            throw new Error(`Pull Request alread exists as: ${active.at(0).title}`)
    ...
    */
    ...
    static PullRequest = class PullRequest {
        ...
        static async from(repository, refresh = false, ttl) {
            ...
            pullrequestCache = await cache.set(pullrequestCache, ttl, 'pullrequests', repository.project.name, repository.name)
