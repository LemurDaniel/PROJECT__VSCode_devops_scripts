...
function activate(context) {
    ...
    disposable = vscode.commands.registerCommand("devopsscripts.pipeline.start", startPipeline)
    context.subscriptions.push(disposable)
...
async function selectOrganization() {
    try {
        ...
        user.currentOrganization = await interaction.userPickOrganization().then(org => org.accountName)
        projectView.instance.refresh()
    } catch (exception) {
        vscode.window.showErrorMessage(exception.message)
    }
...
async function startPipeline(context) {
