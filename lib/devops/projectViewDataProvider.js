...
class ProjectViewDataProvider {
    ...
    async getTreeItem(element) {
        if (element instanceof Project) {
            ...
            const collapsable = element.builds.length > 0
            return new ProjectViewDataProvider.ViewItem(element.name, icon, collapsable, "pipelineContext")
        }
        else if (element instanceof Pipeline.Build) {
            const icon = new vscode.ThemeIcon('sync~spin')
            return new ProjectViewDataProvider.ViewItem(element.sourceBranch, icon, false, "buildContext")
        }
    }
