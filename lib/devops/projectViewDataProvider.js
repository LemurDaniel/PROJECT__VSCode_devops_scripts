...
class ProjectViewDataProvider {
    ...
    async getChildren(element) {
        if (null == element) {
            ...
            const repositories = await element.repositories
            const groupings = utility.groupSimilar(repositories, 4, 8, repo => repo.name)
            if (groupings.groupCount == 1 && groupings.ungrouped.length == 0)
                return repositories

            return [
                ...Object.entries(groupings.grouped)
                    .map(([prefix, repos]) => new ProjectViewDataProvider.ItemContainer(prefix, repos)),
                ...groupings.ungrouped
            ]
