...
async function selectFromOptions(options, label = option => option, description = () => null, canPickMany = true) {
    ...
    if (optionsPoll instanceof Array) {
        return options.filter(option => optionsPoll.map(value => (value.label ?? value)).includes(label(option)))
    } else {
        return options.filter(options => label(options) == (optionsPoll.label ?? optionsPoll))[0]
    }

}


async function selectSingleFromOptions(options, label, description) {
    return await selectFromOptions(options, label, description, false)
}
