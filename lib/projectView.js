...
class ProjectView {
    ...
    async getTreeItem(element) {
        ...
        if (element instanceof Project) {
            ...
            const item = new vscode.TreeItem(element.name.replace('refs/heads/', ''), None)
            ...
            switch (element.result) {
                case Pipeline.Build.RESULTS.none:
                case Pipeline.Build.RESULTS.notStarted:
                case Pipeline.Build.RESULTS.inProgress:
                    item.contextValue = 'buildContextInProgress'
                    break

                case Pipeline.Build.RESULTS.failed:
                case Pipeline.Build.RESULTS.canceled:
                case Pipeline.Build.RESULTS.succeeded:
                case Pipeline.Build.RESULTS.partiallySucceeded:
                    item.contextValue = 'buildContextFinished'
                    break

                default:
                    item.contextValue = 'buildContext'
    ...
    async getChildren(element) {
        ...
        if (null == element) {
            ...
            return element.builds.filter(build => build.showInTreeView)
        }
        else return null
    }
