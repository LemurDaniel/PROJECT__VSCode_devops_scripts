...
class RepositoryView {
    ...
    async select(repository) {
        ...
        if(repository.isDownloadedLocaly) {
            ...
            output.appendLine(`${this.constructor.name} Got Branch '${branch.name}'`)
            this.disposable.title = `Branch '${branch.nameShort}' in '${repository.name}'`
        }
        else {
            this.disposable.title = `Remote '${repository.name}'`
        }
    ...
    async getChildren(element) {
        ...
        if (null == this.repository || !this.repository.isDownloadedLocaly) {
            return []
        }
